...
- [x] Portfolio value summary at top of digest
- [x] Movers & Drivers section with ▲/▼ arrows
- [x] Filter insider transactions < $1M at ingest + editorial prompt
- [x] Sentence-boundary truncation for driver text
- [x] Scraped text deduplication (consecutive lines)
- [x] HTML email renderer (`pipeline/html_renderer.py`)
- [x] Movers JSON parse fallback (`_parse_insights_safe()` with regex recovery)
- [x] Stock data any-time-of-day fix (`period=5d` + most-recent-complete-day)
- [x] NewsData API hang fix (manual executor shutdown)
- [x] `.env` security — keys rotated, file untracked, `.gitignore` fixed

**Current tasks:**
- [ ] 36-hour time-window filter on article relevance (stale articles causing hallucinations)
- [ ] Publication date included in article blocks sent to LLM
- [ ] Portfolio ticker allowlist in editorial prompt (MU, AMAT leaking into digest)
- [ ] ChromaDB stale article cleanup (>7 days) in ingestion pipeline
- [ ] Temporal awareness rule in analyst prompt
...
| Stale article hallucinations | 🔴 Fixing | 36h time-window filter on relevance |
| Non-portfolio tickers | 🔴 Fixing | Portfolio ticker allowlist in editorial prompt |
| Stale Movers drivers | ⚠️ Known | Article date mismatch with today's price direction |
...
9. **Do NOT modify noise_filter.py regex patterns** unless explicitly asked
